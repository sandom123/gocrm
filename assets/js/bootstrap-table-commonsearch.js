...
!function ($) {
    ...
    var initCommonSearch = function (pColumns, that) {
        ...
        //自定义导出
        form.on("click", ".btn-my-export", function (event) {
            that.onColumnCommonExport(undefined, true);
        });
    };
    ...
    var createFormBtn = function (that) {
        var htmlBtn = [];
        var searchSubmit = that.options.formatCommonSearchButton();
        ...
        if(that.options.isShowMyExportBtn){
            htmlBtn.push(sprintf('<button type="button" class="btn btn-danger btn-my-export" >%s</button> ', searchExport));
        }
        htmlBtn.push('</div>');
        htmlBtn.push('</div>');
        return htmlBtn;
    ...
    $.extend($.fn.bootstrapTable.locales[$.fn.bootstrapTable.defaults.locale], {
        ...
        formatCommonSearchButton: function () {
            return "Search";
        },
    ...
    BootstrapTable.prototype.onColumnCommonSearch = function (event, isExport) {
...
//        this.updatePagination();
    ...
    BootstrapTable.prototype.onColumnCommonExport = function (event, isExport) {
            var searchquery = getSearchQuery(this);
            // 追加查询关键字
            this.options.pageNumber = 1;
            this.options.queryParams = function (params) {
            ...
            var params = {filter: JSON.stringify(searchquery.filter), op: JSON.stringify(searchquery.op), op_type: 'export', sort: this.options.sortName, order: this.options.sortOrder};
             params = Object.keys(params).map(function (key) {
                return encodeURIComponent(key) + "=" + encodeURIComponent(params[key]);
            }).join("&");
            window.location.href = this.options.url + "?" + params;
    };
}(jQuery);
